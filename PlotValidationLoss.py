...
def loadValLoss(path="ValidationMetrics.txt"):
    lineRe = re.compile(r"Epoch: (\d+)/\d+, Batch: (\d+)/(\d+), ValLoss: (-?[\d.]+)")
    rows = []
    with open(path) as f:
        for line in f:
            m = lineRe.search(line)
            if m:
                epoch, batch, batchesPerEpoch, valLoss = m.groups()
                step = int(epoch) * int(batchesPerEpoch) + int(batch)
                rows.append((step, float(valLoss)))
    rows.sort()
